...
class Snack():
    badan = []
    kumpulan_arah = {} # sikey = posisi, valuenya = arah pergerakan 
    
    def __init__(self,arena,start,arah_x=1,arah_y=0,warna=(0,0,225)):
        self.arena = arena
        self.start = start
        self.warna = warna
        self.arah_x = arah_x
    ...
    def is_collide(self):
        ...
        is_collide = False;
        for index ,kontak in enumerate(self.badan):
            if index > 0 and pos_kepala == kontak.get_pos():
                is_collide = True
                break
